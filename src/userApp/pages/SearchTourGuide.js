...
const SearchTourGuide = () => {
  ...
  const [showTourGuideData, setShowTourGuideData] = useState(false);
  ...
  const searchTourGuideData = async () => {
  ...
  const renderTourGuideDetails = (doc, index) => (
    <Card key={index} className="mb-4 shadow-sm">
      <Card.Body>
        <Row>
          ...
          <Col xs={12} sm={10}>
            <Card.Header as="h5">TourGuide {doc.spiritualName}</Card.Header>
  ...
  const handleSearchTourGuide = async () => {
    setShowTourGuideData(!showTourGuideData);
    if (!showTourGuideData) {
      await searchTourGuideData();
    }
  };
  ...
  return (
    ...
    <main className="container-fluid mt-5">
      ...
      <div>Search TourGuide Nearby Within 50km Radius</div>
      <Button className="btn btn-primary mt-3" onClick={handleSearchTourGuide}>Search TourGuide</Button>
    
      {showTourGuideData && (
        <div>
          <h3>TourGuide Data</h3>
          {matchingDocs.map((doc, index) => renderTourGuideDetails(doc, index))}
        </div>
      )}
  </main>
    </>
...
export default SearchTourGuide;
